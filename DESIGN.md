...
When I started coding JGemini, as it was formerly called, back in 2020, I was
interested to discover whether it was really possible to implement a workable
client "in a weekend", as the founders of Gemini claimed. It turns out that it
was, but it wasn't a very _good_ client. I made a number of design decisions
that I wouldn't have made, had I been planning something that might have taken
a year.  Or, as it turns out, six years.  It's not clear to me that these were
entirely stupid decisions, but I doubt I would make them again.
...
protocol Caztor supports, plus all the factories and glue code to make the JVM
use them. `URLConnection` is an ugly class, because it doesn't have a specific
"close" method that other classes can call. 

What this means is that it's relatively easy for Caztor to leak connections,
...
So what Caztor really consists of are an HTML viewer, a set of classes that
...
Another fundamental design decision was that Caztor be capable of distribution
...
the asynchronous work in Caztor is done in anonymous subclasses of
`SwingWorker`. 
...
Caztor only handles a small number of document types -- Gemtext, Markdown,
...
So Caztor will have issued the request, and started to read the response,
before it knows whether it can handle it, or what the user wants to do if it
can't. Content that Caztor _can_ handle needs to be loaded into memeory;
...
The way Caztor handles this situation is to read the response as far as the
...
Caztor could begin the transfer, then leave the connection open whilst
...
decision. So, while the way Caztor deals with foreign content is sub-optimal,
I don't really see a better way, given the other fundamental design decisions. 

## Streaming

I'm not going to implement a media player in Caztor -- there are already
...
Instead, Caztor just opens a connection to the server, starts an external
...
the viewer. Caztor uses the same `SwingWorker` approach for downloading this
...
There's also a problem in coordinating the life-cycles of the Caztor
application and the external player. This is hard to get right, and I'm aware
that I haven't, not entirely. Sometimes media playback will prevent Caztor
closing cleanly. At other times, closing the player will result in annoying
error messages in the Caztor UI.

There isn't a huge amount of streaming content in the Gemini world at present.
If that changes, I might have to rethink how Caztor handles streaming.

## Feed aggregation

Caztor mostly handles gemsub-style feeds. These are Gemtext pages with
...
the Gemini world. Caztor has a "Feed aggregator" dialog box that
