...
## Obtaining the Caztor binary

Get the Caztor JAR file [from GitHub](https://github.com/kevinboone/caztor/releases).
...
## Running Caztor on Linux

You can run Caztor from the command prompt like this:

    java -jar /path/to/caztor-x.y.jar

where x.y is the version number (currently 1.0).

If your system associates Java JAR files with the `.jar` extension, you might
be able to launch Caztor from a file manager or program manager or whatever.
If you're running from a command line, you can specify a URL or file to load:

    java -jar /path/to/caztor-1.0.jar file:///path/to/file.gmi

or

    java -jar /path/to/caztor-1.0.jar gemini://host:port/path.gmi

For convenience, local filenames don't need a full URL -- just the filename
will do. Caztor will expand it to a URL internally. If the name of a local
...
## Running Caztor on Windows 

If you installed the Java JVM using the executable installer, just double-click the
Caztor JAR file in whatever directory you saved it. You could create a link to
