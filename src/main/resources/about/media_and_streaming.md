...
Caztor has no built-in media player. However, it does have features that make
it easier to use an external player with media streams.  `ffmpeg` and `vlc`
both work with Caztor on Linux, and at least some versions of these
applications work on at least some versions of Microsoft Windows.

It's not problematic to play a media _file_: just download it using Caztor,
...
When you click a link to a media document in Caztor's viewer, you'll be asked
what to do with it. Options including saving a file, and streaming to a player.
There is no way Caztor can make the determination itself, because none of the
protocols that Caztor supports provide any content-length indication. Caztor
...
Consequently, Caztor leaves this decision to the user.

If you choose to stream the file, then Caztor will run the selected media
...
Caztor keeps running, or until you cancel the transfer in the download
manager.
...
Although Caztor does work with players that have no graphical user interface,
...
Caztor won't stop you opening multiple media streams, but your player might. 
