...
Caztor has preliminary support for subscribing to feeds. Its feed aggregator
...
to ask Caztor to aggregate other aggregators. Caztor de-duplicates posts, so
...
Anything that Caztor can treat as a feed will contain a list of links, each one
with a date in YYYY-MM-DD format at the start of the link text. It's not
a problem if the document contains other content, but Caztor only reads the
links.

It shouldn't harm Caztor if you subscribe to a page that is not, in fact,
...
When you view the aggregated posts, Caztor just shows the generated
...
`feeds.gmi` in the `.caztor` directory.  You can change this location in the
configuration file, if you wish.

You can add, re-order, or delete feeds in the subscriptions list using an
ordinary text editor (with Caztor not running), or using the built-in file
...
file, and then restarting Caztor after editing it.
...
configure Caztor to do it every time it starts. Use the Feeds tab of
...
Caztor supports feeds in Atom and "gmisub" format. It will subscribe to any
