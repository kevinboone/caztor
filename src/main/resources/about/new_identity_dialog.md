...
>*Note*  
> If you're unsure what Caztor means by "identity", or what
> a client certificate is, please see 
> the page on [identity](identity.md) first.

You can use the _New identity_ dialog to have Caztor create a new client
...
* The identity name. This can be any combination of letters, digits,
  ...
  Caztor; it's the name that appears in the identity list in
  the [Set identity](set_identity_dialog.md) dialog. Caztor will use
...
* The name for the certificate. This must be in the form of an X.509
  ...
  and Caztor fills in the "CN=" part for you.
* A password, which will be used to protect the keystore that Caztor
  generates. 
...
Caztor's certificate manager can only create self-signed certificates
