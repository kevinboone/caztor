...
> *Note*  
> You can manage most of Caztor's configuration using the main user interface,
...
editor by selecting "Save" (ctrl+S), Caztor saves the contents of the editor
...
On Linux, Caztor will read a system-level properties file, if it exists, and
...
The name of the user configuration file is `caztor.properties`, in the
directory `.caztor`, in the location that the Java JVM recognizes as the
...
configuration file will usually be
    ...
    $HOME/.caztor/caztor.properties

on Linux, and probably

    c:\users\{username}\.caztor\caztor.properties
...
in the `.caztor` directory.

`bookmark.file`  
Set a custom bookmark filename, rather than `bookmarks.gmi` in the `.caztor`
directory. 
...
has subscribed. The default is `feeds.gmi` in the `.caztor` 
directory. 
...
every time Caztor starts. The default is 'no', so you'll need
...
is saved, it will be to the file `$HOME/.caztor/caztor.history`, unless you
...
`caztor.history` in the `.caztor` directory.
...
tell Caztor to stream a file, rather than download it. This setting is mostly
...
`ui.icon.size`  
The size of toolbar icons in pixels. You will need to restart Caztor
