# Caztor -- using client certificates

>*Note*  
> If you're unsure what Caztor means by "identity", or what a client
...
As with everything else in Caztor, you can control which certificate to send
...
For first steps using Caztor's
...
certificates in Caztor, should you need to do it manually.
...
Caztor supports per-host assignment of client certificates, but not per-page
...
what the certificates are named within the keystore, because Caztor assumes that
...
that Caztor needs to know about. You can put this in any convenient directory. 

## Using an existing keystore with Caztor
...
You can also specify a fall-back identity, which Caztor will use if no other
...
Caztor won't send a certificate to that host.

What if you want to send a client certificate to every host _except_
some few? Caztor caters for this situation as well. We define a fallback
