...
Caztor is supplied as a single Java JAR file. It requires no specific
...
## Running Caztor without installing

So long as you have a Java JVM installed, you can run Caztor from a prompt
like this:
...
be able to launch Caztor from a file manager or program manager, just by
clicking the JAR.

If you're running from a command line, you can specify a URL or file to load:

    java -jar /path/to/caztor-2.0.jar gemini://larsthebear.me/ 
...
You'll need to create a script or batch file that launches Caztor using the
...
None of this installation is necessary if you just want to run Caztor from a
command prompt.
