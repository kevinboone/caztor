# Handling non-document files in Caztor 

Caztor's viewer window handles text and images. Of course, small-net documents,
...
- "Hand off to desktop". Caztor saves the document to a temporary file, 
  and invokes the desktop's default action for it. The desktop will
  usually launch an application; Caztor doesn't allow you to choose
  the application, but your operating system probably does.
- "Prompt for a filename and save". You can choose where to save the
  file.
- "Save without prompting". Caztor saves the file in a default location,
  choosing a new filename if a file of the same name exists. The default
  download location is usually `$HOME/.caztor/downloads`.
- "Stream to player". Caztor will run an application and pass the 
...
want Caztor to perform the same action for the same document type in future.
...
allows a choice for documents that Caztor supports internally: you can open
