...
> page on [identity](identity.md) in Caztor.
...
> *Note*  
> Although you'll be viewing a particular page or URL, Caztor assigns identity
...
If you don't assign any identity to a host -- which is the default -- Caztor
...
Technically, a Caztor identity is a Java keystore containing a client
...
* The filename of a Java keystore. This must be a file that already exists,
  whether you created it in Caztor or in some other way. You can use the
  file picker to navigate to the keystore file.
* A password. This can be any sequence of characters. Don't use a password
  that you use for anything else, as Caztor doesn't go to great lengths
  to conceal it. 
...
existing Java keystore -- which may not have been created in Caztor -- or to
have Caztor create the client certificate and keystore from scratch.
