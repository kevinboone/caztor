...
see a list of possible identities; if this is your first time using Caztor,
...
Assign a password to the keystore that Caztor is going to generate.  You don't
necessarily need to remember it, since Caztor will store it. Knowing the
password is only important if you plan to manipulate the keystore with other
tools.

When you click _Submit_, all being well, Caztor will generate the
...
Please note that Caztor's identity manager is somewhat rudimentary.  It can
