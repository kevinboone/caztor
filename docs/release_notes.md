...
the aggregated posts in date order. And, of course, all the names of
everything have been changed from the JGemini naming.

## JGemini version 3.0.0
...
manager with its own user interface.  Caztor no longer assumes that every
request will result in a response that will fit entirely in memory -- if you
choose to save a file, Caztor will transfer it in the background, and you can
...
completed, and Caztor will now resist being closed when there are ongoing
transfers.
...
* Caztor now supports Gopher, nex, and Spartan, as well as Gemini
