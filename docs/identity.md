...
Caztor creates a client certificate, it always creates a self-signed one.
...
to be stored somewhere, in some recognized format.  Caztor, being a Java
program, stores certificates in Java keystores. Java supports several different
types of keystore and Caztor can read them all. But what it writes, when it
...
format Java understands, and tell Caztor to use it.  You might have to do that
...
contain many different certificates with many different purposes, Caztor
doesn't expect to find more than one client certificate in any keystore.
Caztor will always use the first certificate it finds in a keystore,
regardless how many there are. 

As a result, creating a new "identity" in Caztor amounts to creating a
